...
################################################################################
  ...
  output$appltext_cit = renderText({
    paste("Total asylum applications in", input$ctz, "in the period", input$time3[[1]], "-",input$time3[[2]], ": ", sum(df_sub_31()$Applications))
  })
  ...
  output$firsttext_cit = renderText({
    req(df_sub_22())
    a = paste("Total asylums granted by", input$ctz, "by first istance in the period", input$time3[[1]], "-",input$time3[[2]], ": ", sum(df_sub_32()$Total_pos_first))
    b = "Of which by: "
  ...
  output$deftext_cit = renderText({
    req(df_sub_33())
    a = paste("Total asylums granted by", input$geo3, "by definitive decision in the period", input$time3[[1]], "-",input$time3[[2]], ": ", sum(df_sub_33()$Total_pos_def))
    b = "Of which by: "
